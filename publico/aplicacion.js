(function () {
  ...
  function configurarCamara() {
    ...
    $("#configurarCamaraEsp32")?.addEventListener("click", configurarCamaraEsp32);
  ...
  function mostrarBiometriaRecluta() {
    ...
    vista.innerHTML = imagen ? `<img src="${imagen}" alt="Imagen biométrica capturada">` : "Sin captura biométrica";
  ...
  function mostrarPerfilEgresadoRecluta() {
    ...
    vista.innerHTML = `
      ...
      <p><strong>Día de inducción:</strong> ${postulacionActual.fechaInduccion || "Por definir"}</p>
      ${postulacionActual.biometria?.imagenBase64 ? `<img src="${postulacionActual.biometria.imagenBase64}" alt="Imagen biométrica de ${postulacionActual.nombreRecluta}">` : `<div class="preview">Sin imagen biométrica</div>`}
  ...
  async function iniciarVerificacionSeguridad(postulacion) {
    ...
    $("#comparacionSeguridad").innerHTML = `
      <article class="biometria-item">
        ${postulacion.biometria?.imagenBase64 ? `<img src="${postulacion.biometria.imagenBase64}" alt="Biometría guardada">` : `<div class="preview">Sin biometría guardada</div>`}
    ...
    mensaje("ESP32-CAM conectada. Captura el rostro y compara las imagenes.");
  }

  function obtenerUrlEsp32(pedir = false) {
    let urlEsp32 = localStorage.getItem("ContrataT-esp32cam-url") || "";
    if (!urlEsp32 && pedir) {
      urlEsp32 = prompt("Escribe la URL que mostro el ESP32-CAM en el Monitor serial:", "http://192.168.1.50") || "";
  ...
  function configurarCamaraEsp32() {
    ...
    if (!urlEsp32) {
      ...
      mensaje("Se borro la configuracion de la camara.");
      return;
    }
    localStorage.setItem("ContrataT-esp32cam-url", urlEsp32);
    iniciarVistaEsp32(urlEsp32);
    mensaje("Direccion de la ESP32-CAM actualizada.");
  }
  ...
  function iniciarVistaEsp32(urlEsp32) {
    ...
    const actualizar = () => {
      const vista = $("#videoSeguridad");
      if (vista) vista.src = `${urlEsp32}/captura?ts=${Date.now()}`;
    };
    actualizar();
    temporizadorCamaraEsp32 = setInterval(actualizar, 1200);
  }

  async function capturarRostroSeguridad() {
    ...
    if (!urlEsp32) return;
    try {
      const respuesta = await fetch(`${urlEsp32}/captura?ts=${Date.now()}`, { mode: "cors" });
      if (!respuesta.ok) throw new Error("La ESP32-CAM no devolvio una imagen");
      const imagen = await blobADataUrl(await respuesta.blob());
      $("#capturaActualSeguridad").innerHTML = `<img src="${imagen}" alt="Rostro actual capturado por ESP32-CAM">`;
      accesoSeleccionado.capturaSeguridad = imagen;
      mensaje("Captura de ESP32-CAM lista. Compara visualmente y confirma.");
    } catch (error) {
      mensaje("No se pudo capturar desde ESP32-CAM. Revisa IP, WiFi y energia.");
    }
  }
  ...
  async function validarSeguridad(coincide) {
    ...
    if (coincide && !accesoSeleccionado.capturaSeguridad) {
      mensaje("Primero captura el rostro actual con la ESP32-CAM.");
      return;
    }
    if (coincide) {
      const puertaAbierta = await abrirPuertaEsp32Cam();
  ...
  async function abrirPuertaEsp32Cam() {
    ...
    try {
      const respuesta = await fetch(urlEsp32 + "/abrir", { method: "GET", mode: "cors" });
      if (!respuesta.ok) throw new Error("El ESP32-CAM rechazo el comando");
  ...
  function renderizarAccesosSeguridad(accesos) {
    ...
    const mostrar = (postulacion) => {
      $("#detalleAccesoSeguridad").innerHTML = `
        ...
        ${postulacion.biometria?.imagenBase64 ? `<img src="${postulacion.biometria.imagenBase64}" alt="Biometría de ${postulacion.nombreRecluta}">` : `<div class="preview">Sin biometría</div>`}

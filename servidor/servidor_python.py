...
def enviar_token(destino, token, portal, nombre):
    ...
    mensaje["Subject"] = f"Token de verificación ContrataT - {portal.upper()}"
    mensaje["From"] = remitente
    mensaje["To"] = destino
    mensaje.set_content(f"Hola {nombre or 'usuario'}, tu token de verificación es {token}. Expira en 10 minutos.")
    ...
    return {"modo": "correo", "mensaje": "Token enviado al correo electrónico."}
    ...
    def do_PUT(self):
        ...
        try:
            if ruta.startswith("/api/postulaciones/") and ruta.endswith("/biometria"):
                ...
                col("postulaciones").update_one({"_id": post_id}, {"$set": {"estado": "Biometria pendiente de revision RH", "mensajeAutomatico": "RH revisará tu imagen biométrica. Si no es validada deberás capturarla de nuevo antes de agendar entrevista.", "razonRechazo": "", "requiereNuevaBiometria": False, "actualizadaEn": ahora()}})
        ...
        try:
            ...
            if ruta.startswith("/api/rh/postulaciones/"):
                ...
                if estado == "Acceso autorizado por RH":
                    campos["mensajeAutomatico"] = "RH validó tu CV para entrevista. Registra tus datos biométricos para agendarte una entrevista."
                    campos["requiereNuevaBiometria"] = True
                if estado == "Biometria rechazada por RH":
                    campos["mensajeAutomatico"] = "RH no validó tu imagen biométrica. Entra a Datos biométricos y vuelve a capturar tu rostro con buena luz y de frente para poder agendar la entrevista."
                    campos["requiereNuevaBiometria"] = True
                if estado == "Acceso listo para Seguridad":
                    campos["mensajeAutomatico"] = "RH validó tu imagen biométrica. Tu entrevista ya fue agendada y tu acceso está disponible para Seguridad."
